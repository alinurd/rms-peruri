...
class Officer extends BackendController {
	...
	public function __construct()
	{
		...
		$this->set_Open_Tab('Data Owner');
			...
			$this->addField(array('field'=>'nip', 'search'=>true, 'size'=>50, 'required'=>true));
			...
			$this->addField(array('field'=>'user_name', 'type'=>'free', 'search'=>true, 'size'=>20));
			$this->addField(array('field'=>'sandi', 'type'=>'free', 'input'=>'pass', 'size'=>20));
			$this->addField(array('field'=>'sandic', 'type'=>'free', 'input'=>'pass', 'size'=>20));
			$this->addField(array('field'=>'user',  'type'=>'free','search'=>true, 'size'=>50));
			$this->addField(array('field'=>'status','input'=>'boolean', 'size'=>50));
			$this->addField(array('field'=>'sts_owner', 'input'=>'boolean', 'size'=>50));
		$this->set_Close_Tab();
	...
	function listBox_USERx($row, $value){
		$group='';
		$no=0;
		foreach($this->data_group as $dg){
			if ($dg['user_no']==$row['l_id_user']){
				if($no==0)
					$group .= $dg['group_name'];
				else
					$group .= "/" . $dg['group_name'];
	...
	function POST_UPDATE_PROCESSOR($id , $new_data, $old_data){
		...
		if (!empty($new_data['l_sandi']))
			$password = $this->authentication->change_password($new_data['l_sandi']);
