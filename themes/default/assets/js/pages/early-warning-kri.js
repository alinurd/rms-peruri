...
console.log(data);
	...
	$(document).on("click", "[id^=simpan_realisasi_]", function () {
		...
		var realisasi = $("input[name='realisasi" + id + month + "']").val();
 	
		var data = { 'mode': 'kri','id': id, 'bulan': month, 'realisasi': realisasi };
		...
		// }
  		// var parent = $(this).parent();
		var target_combo = "";
		var url = modul_name + "/simpan-realisasi-kri";
...
function result_realisasi_kri(hasil) {
  ...
  if(hasil.bg) {
    ...
    $('#kri-' + hasil.id + hasil.bulan).css('background-color', hasil.bg); 
}
