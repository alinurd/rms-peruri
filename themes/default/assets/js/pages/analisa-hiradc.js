$(function(){
	...
	$(document).on("click","#addMitigasi", function(){
		...
		var tingkat_resiko = $("input[name=tingkat_resiko_tmp]").val();
		var id_edit=0;
		var data={'parent':parent_no, 'id':id_detail, 'id_edit':id_edit, 'tingkat_resiko':tingkat_resiko};
	...
	$(document).on("click","#saveMitigasi", function(){
		var oper = $("#operasional").val();
		var sekarang = $("#sekarang").val();
		var mendatang = $("#mendatang").val();
		var tgl_mulai = $("#tgl_mulai").val();
		var tgl_selesai = $("#tgl_selesai").val();
		var program = $("#program_no").val();	
		var faktor = $('input[name="pertimbangan[]"]').val();
		var parent_no=$('input[name="parent_mitigasi"]').val();
		var id_detail=$('input[name="id_detail_mitigasi"]').val();
		var id_edit=$('input[name="id_edit_mitigasi"]').val();
		
		var data = {'operasional':oper,'sekarang':sekarang,'mendatang':mendatang,'program':program,'pendukung':faktor,'parent':parent_no,'id_detail':id_detail,'id_edit':id_edit,'tgl_mulai':tgl_mulai,'tgl_selesai':tgl_selesai};
		var parent = $(this).parent();
		var target_combo = "";
...
function get_severity(hasil){
	...
	$('input[name="severity_no"').val(hasil.severity_no);
