$(function(){
	...
	$("#owner_no, #period_no,#bulan,#bulanx,#datetimepickerx").change(function(){
		...
		var tanggal = date.getFullYear() + '-' + ("0" + (date.getMonth() + 1)).slice(-2) + '-' + ("0" + date.getDate()).slice(-2) + ' ' + ("0" + date.getHours()).slice(-2) + ':' + ("0" + date.getMinutes()).slice(-2);
		...
		var id = parseFloat(id_owner) + "-" + parseFloat(id_period) + "-" + parseFloat(bulan) + "-" + parseFloat(bulanxx) + "-" + parseFloat(tanggal) + "-" + parseFloat(type_dash);
		$(".tahun2").text(tahun2);
		$(".bulan2").text(bulan2);
		$(".bulanx").text(bulanx);
		...
		var data = { 'id_owner': id_owner, 'id_period': id_period, 'bulan': bulan, 'bulanx': bulanxx, 'tanggal': tanggal };
		console.log(data)
	...
	$(document).ready(function () {
		...
		var data = { 'id_owner': id_owner, 'id_period': id_period, 'bulan': bulan, 'tanggal': tanggal, 'bulanx': bulanxx };
		console.log(data)
		var target_combo = $("#mapping");
		var url = modul_name + "/get_map";
	})
...
function hoho(e) {
		...
		var bulan = $("#bulan").val();
			var bulanx = $("#bulanx").val();
 			// console.log(kel + '2')
		var data = { 'id': id, 'owner': owner, 'tahun': tahun, 'bulanx': bulanx, 'bulan': bulan, 'kel':kel};
// console.log(data)
