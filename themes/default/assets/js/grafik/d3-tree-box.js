...
function treeBoxes(urlService, jsonData, target)
{
	var urlService_ = '';
	...
	init(urlService, jsonData, target);

	function init(urlService, jsonData, target)
	{
	...
	function drawTree(jsonData, target)
	{
		...
		getMouseWheelEvent(target);
		d3.select('#'+target).select('svg').on(mouseWheelName, null);
		d3.select('#'+target).select('svg').on('dblclick.zoom', null);
	...
	// 1) ******************* Update the nodes *******************
		...
		.attr('height', rectNode.height + 50)
		...
		.attr('height', function() {
					return (rectNode.height + 50 - rectNode.textMargin * 2) < 0 ? 0
							: (rectNode.height + 50 - rectNode.textMargin * 2)
				})
	...
	function removeMouseEvents(target) {
		// Drag and zoom behaviors are temporarily disabled, so tooltip text can be selected
		mousedown = d3.select('#'+target).select('svg').on('mousedown.zoom');
		d3.select('#'+target).select('svg').on("mousedown.zoom", null);
	}
	
	function reactivateMouseEvents(target) {
		// Reactivate the drag and zoom behaviors
		d3.select('#'+target).select('svg').on('mousedown.zoom', mousedown);
	}
	
	// Name of the event depends of the browser
	function getMouseWheelEvent(target) {
		if (d3.select('#'+target).select('svg').on('wheel.zoom'))
		{
			mouseWheelName = 'wheel.zoom';
			return d3.select('#'+target).select('svg').on('wheel.zoom');
		}
		if (d3.select('#'.target).select('svg').on('mousewheel.zoom') != null)
		{
			mouseWheelName = 'mousewheel.zoom';
			return d3.select('#'+target).select('svg').on('mousewheel.zoom');
		}
		if (d3.select('#'+target).select('svg').on('DOMMouseScroll.zoom'))
		{
			mouseWheelName = 'DOMMouseScroll.zoom';
			return d3.select('#'+target).select('svg').on('DOMMouseScroll.zoom');
		}
	}
