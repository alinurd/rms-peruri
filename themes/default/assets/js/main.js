...
$(function () {
  ...
  $(document).on("change", ".rupiah", function () {
    ...
    var value = accounting.formatMoney(nilai, "", 1);
    $(this).val(value);
  });
  ...
  $(document).on("click", ".action_tab", function () {
    var id = $(this).data("id");
    console.log(id);
    $('#tab_input a[href="' + id + '"]').tab("show");
  });
});
...
$(window).on("load", function (e) {
  $(".div1").width($(".table-scroll").width());
  $(".div2").width($(".table-scroll").width());
});
...
function selectCheck() {
  ...
  $("[target-radio], label[target], radio-name[target]").click(function (e) {
    ...
    if ($type == "multiple") var $checkbox = $('input[name="' + $target + '"]');
    $('input[type="checkbox"]').next().removeClass("input-error");
    $('input[type="radio"]').next().removeClass("input-error");
  ...
  if ($.isFunction($.fn.dataTable)) {
    oTable = $("table#" + idtbl).dataTable({
      ...
      aLengthMenu: [
        ...
        [5, 10, 15, 25, 50, 100, 200, 500, 1000, "All"],
      ],
      oLanguage: {
        sProcessing: "",
    ...
    $("table#" + idtbl + ' th input[type="checkbox"]')
      .parents("th")
      .unbind("click.DT");
...
function cari_ajax_combo(
  ...
  if (typeof pesan == "undefined") pesan = "Error Load Database";
  if (typeof title_msg == "undefined") title_msg = "Error";
...
function cari_ajax_singgle(tipe, data, url) {
  ...
  $.ajax({
    ...
    dataType: "json",
    success: function (result) {
      pesan_toastr("Mohon tunggu ", "success", "Sukses", "toast-top-center");
    },
    error: function (msg) {
      console.log(msg);
      // alert(msg.statusText+": "+msg.status);
      // if (sts_loading)
      // 	stopLooding(parent);
      // pesan_toastr('Terjadi Kesalahan, mohon periksa kembali data yg dimasukkan', 'err', 'Error', 'toast-top-center');
    },
    complete: function () {},
  });
}
function cari_ajax_singgle_del(tipe, data, url) {
  url = base_url + url;
  ...
  $.ajax({
    ...
    error: function (msg) {
      if (msg) {
        ...
        // console.log(msg);
        // console.log('status=>'.msg.statusText + ": " + msg.status);
        // console.log('cari_ajax_singgle_del-err');

        // If the response is '1', reload the page
        // location.reload(true);
      } else {
        // Handle any other response here if needed
...
function cari_ajax(parent, spinner, target, url) {
  ...
  if (typeof spinner != "undefined") show_spinner(spinner);
  ...
  $.ajax({
    ...
    complate: function () {
      if (typeof spinner != "undefined") close_spinner(spinner);
    },
  });
}
...
function remove_install(t, iddel, kel) {
  console.log(iddel);
  if (typeof kel == "undefined") var kel = "";
...
function remove_installx(t, iddel, kel) {
  console.log(iddel);
  if (typeof kel == "undefined") var kel = "";
      ...
      $.ajax({
        ...
        success: function (result) {
          if (result.sts == "0") {
            ...
            t.parentNode.parentNode.parentNode.deleteRow(ri - 2);
            pesan_toastr(result.ket, "info", "Info", "toast-top-center");
          }
          stopLooding(t);
        },
...
function cari_ajax_combo_file(
  ...
  $.ajax({
    ...
    error: function (msg) {
      ...
      pesan_toastr("Data gagal disimpan", "err", "Error", "toast-top-center");
    },
    complete: function () {},
  });
}
