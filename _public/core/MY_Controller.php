...
class FrontendController extends MX_Controller
{
	...
	public function get_script_template($nm_modul=''){
		...
		$this->template->set_partial('sidebar_frontend_left','sidebar_frontend_left', array('data'=>$stat));
	}
	
	function get_combo($kel, $param=array(), $param2=''){
		$result = $this->data->get_combo_model($kel, $param, $param2);
		return $result;
	}
	
	function get_combo_no_select($kel, $param=array(), $param2=''){
		$this->data->set_NoSelect(false);
		$result = $this->data->get_combo_model($kel, $param, $param2);
...
class BackendController extends MX_Controller
{
	...
	public function __construct() 
	{
		...
		if (!$this->authentication->is_loggedin() && $this->router->fetch_module()!=='auth')
		{
			...
			// doi::dump(current_url());
			$basedata = current_url();

			// Membuat URL dengan data sebagai parameter
			$redirect_url = base_url('auth/season') . '?basedata=' . urlencode($basedata);
			// die();
if(!$base){
				echo "<script>
...
alert('Session anda telah berakhir. Silahkan login kembali.');
	...
	public function __delete()
	{
		...
		if (method_exists($this->router->fetch_class(),'POST_DELETE_MANUAL')){
			...
			if ($check_before){
				...
				$this->db->trans_begin();
				if (!empty($this->tbl_simpan)){$this->tmp_data['primary']['tbl']=$this->tbl_simpan;}
				...
				if ($result)
				{
					if (method_exists($this->router->fetch_class(),'POST_DELETE_PROCESSOR'))
					{
						...
						$result=$this->POST_DELETE_PROCESSOR($id);
					}
				}
				if($result){
			...
			else
			{
				...
				if (! $check_before){
					...
					if($id_new && $result){
						...
						if ($save=='Simpan' || $save=='Simpan_Quit'){
							if (method_exists($this->router->fetch_class(),$method_name)){
								$url=$this->$method_name(base_url($this->_Snippets_['modul'].'/edit/'.$id_new), $id_new, $save);
							}elseif($save=="Simpan"){
								$url = base_url($this->_Snippets_['modul'].'/edit/'.$id_new);
							}elseif($save=="Simpan_Quit"){
	...
	public function __print()
	{
		...
		if (method_exists($this->router->fetch_class(),'PRINT_CUSTOM')){
			...
			$optional=[];
			if (method_exists($this->router->fetch_class(),'PRINT_OPTIONAL')){
				$optional = $this->PRINT_OPTIONAL($data);
			}
			
			$sts_cetak=$this->cetak->cetak_pdf_satu($data, $this->tmp_data['title'], $nmfile, array(), $arr_title, true, $this->tmp_data['fields'], $optional);
		}
	}
	...
	function listBox_SORT($row, $value){
	...
	function set_Tbl_Master_Child($tbl){
		$tbl=str_replace($this->db->dbprefix,'', $tbl);
	...
	function set_Table_List($nmtbl, $field, $title='', $size=0, $align='left', $show=true, $print=true){
		if (empty($title)){
			if (defined(strtoupper('_'.$nmtbl.'_'.$field.'_'))){
	...
	function _CHANGE_TABLE_MASTER($tbl){
		$this->tbl_simpan=$tbl;
	}
	...
	function _getPrivilege($aksi='add'){
	...
	function _CHECK_PRIVILEGE_OWNER($tbl='form_individu', $field="owner_no"){
	...
	function insertBox_PETA($field){
		$result = $this->data_peta();
		return $result;
	}
	
	function updateBox_PETA($field, $row, $value){
