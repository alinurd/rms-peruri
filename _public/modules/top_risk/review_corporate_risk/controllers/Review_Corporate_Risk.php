...
class Review_Corporate_Risk extends BackendController {
	var $type_risk=0;
	var $table = "";
	var $post = array();
	var $sts_cetak = false;
	...
	public function __construct()
	{
		...
		$this->set_Open_Tab('Review Top Risk');
		...
		$this->set_Table_List($this->tbl_master, 'name','Nama Area',15);
		$this->set_Table_List($this->tbl_master, 'kategori_no','Kategori');
		$this->set_Table_List($this->tbl_master, 'event_name','Resiko Peristiwa');
		// $this->set_Table_List($this->tbl_master, 'risk_couse_no','Penyebab');
		// $this->set_Table_List($this->tbl_master, 'risk_impact_no','Akibat');
		$this->set_Table_List($this->tbl_master, 'inherent_analisis_id','Risk Level',15);
