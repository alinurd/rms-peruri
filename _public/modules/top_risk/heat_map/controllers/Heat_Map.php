...
class Heat_Map extends BackendController {
	var $tmp_data=array();
	...
	{	
		...
		$data['mapping']=$this->data->get_map_rcsa();
	...
	function get_map(){
		...
		$data['mapping']=$this->data->get_map_rcsa($post);
		$hasil['combo'] = $data['mapping'];
		echo json_encode($hasil);
	}
	
	function get_detail_map(){
		$post=$this->input->post();
		$rows=$this->db->where('inherent_level', $post['id'])->get(_TBL_VIEW_TOP_RISK)->result_array();
