...
class Approve_Admin extends BackendController
{
	var $tmp_data = array();
	...
	public function __construct()
	{
		...
		$this->set_Open_Tab('RCSA');
			...
			$this->addField(array('field' => 'anggaran_rkap', 'type'=>'float', 'size' => 100, 'required' => true, 'search' => true));
			$this->addField(array('field' => 'owner_pic', 'size' => 100, 'search' => true));
		...
		$this->set_Table_List($this->tbl_master, 'periode_name','Periode');
		$this->set_Table_List($this->tbl_master, 'sts_propose_text', 'Status Propose', 0, 'center');
		$this->set_Table_List($this->tbl_master, 'register', 'Risk Register', 8, 'center');
	...
	function propose()
	{
		...
		$data['field']['date_approve_admin'] = date('Y-m-d');
		$data['field']['user_approve_rm'] = $this->authentication->get_Info_User("identifier");
	...
	function simpan_reject()
	{
		...
		$data['field']['note_approve_admin'] = $note;
		$data['field']['date_approve_admin'] = date('Y-m-d');

		$current_time = time();
		$new_time = $current_time - (7 * 3600); 
		$create_date = date('Y-m-d H:i:s', $new_time);	
		...
		$this->db->update($data['table'], $data['field']);
		$sts = $this->db->affected_rows();
		$this->db->insert('log_propose', ['create_date' => $create_date,'rcsa_no' => $id_rcsa, 'note' => $note, 'petugas_no' => $this->authentication->get_Info_User("identifier"), 'keterangan' => 'Revisi from RM to Risk Agent', 'create_user' => $this->authentication->get_Info_User('username')]);
		echo json_encode('Success');
	}
}
