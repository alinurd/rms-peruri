...
class Rcsa_Context extends BackendController
{
	...
	function simpan_copy()
	{
		...
		foreach ($arrsasaran as $indexarrayx => $sasaran) {
			foreach ($sasaran as $indexarrayx => $data) {
				if (
					$indexarrayx !== 'create_user' &&
					$indexarrayx !== 'id'
				) {
					$upSasaran[$indexarrayx] = $data;
					$upSasaran['rcsa_no'] = $rcsa_no;
					$upSasaran['create_user'] = $this->authentication->get_info_user('username');
				}
			}
			$sasaran_no = $this->crud->crud_data(array('table' => _TBL_RCSA_SASARAN, 'field' => $upSasaran, 'type' => 'add'));
		...
		foreach ($arrdetail as $detail) {
			foreach ($detail as $indexarray => $data) {
				if (
					...
					$updetail['sasaran_no'] = $sasaran_no;
