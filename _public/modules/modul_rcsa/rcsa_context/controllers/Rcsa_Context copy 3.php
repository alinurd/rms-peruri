...
class Rcsa_Context extends BackendController
{
	...
	function simpan_copy()
	{
		...
		// 		$arrsasaranx = $this->db->where('rcsa_no', $post['id'])->get(_TBL_RCSA_SASARAN)->result_array();
		// 		foreach ($arrsasaranx as $sasaranx) {
		// 			$upSasaranx = [];
		// 			if($id!=$sasaranx['id']){
		// 			foreach ($sasaranx as $index => $data) {
		// 				if (!in_array($index, ['create_user', 'event_no', 'kategori_no', 'sub_kategori', 'detail_no', 'id'])) {
		// 					$upSasaranx[$index] = $data;
		// 				}
		// 			}
					
		// 		$upSasaranx['rcsa_no'] = $rcsa_no;
		// 		$upSasaranx['create_user'] = $this->authentication->get_info_user('username');
		// 		$this->crud->crud_data(array('table' => _TBL_RCSA_SASARAN, 'field' => $upSasaranx, 'type' => 'add'));
		// 	}

		// 	}
		// }
// Ambil data sasaran dari database
$arrsasaranx = $this->db->where('rcsa_no', $post['id'])->get(_TBL_RCSA_SASARAN)->result_array();
$arrsasaran = $this->db->where('rcsa_no', $post['id'])->get("bangga_view_sasaran_detail")->result_array();
$sasaran_no = [];
$sasaran_nox = [];
...
foreach ($arrsasaran as $sasaran) {
    ...
    $id = $sasaran['id'];
    $namSasaran = $sasaran['sasaran'];

    if (!isset($sasaran_no[$id])) {
        // Simpan data baru jika belum ada di $sasaran_no
        $upSasaran['rcsa_no'] = $rcsa_no;
        $upSasaran['create_user'] = $this->authentication->get_info_user('username');
        $this->crud->crud_data(['table' => _TBL_RCSA_SASARAN, 'field' => $upSasaran, 'type' => 'add']);
        $sasaran_no[$id] = $this->db->insert_id();
    } else {
        // Cek data sasaran yang ada di $arrsasaranx
        foreach ($arrsasaranx as $sasaranx) {
            if ($namSasaran !== $sasaranx['sasaran']) {
                $upSasaranx = [];
                foreach ($sasaranx as $index => $data) {
                    if (!in_array($index, ['create_user', 'event_no', 'kategori_no', 'sub_kategori', 'detail_no', 'id'])) {
                        $upSasaranx[$index] = $data;
                ...
                if (!isset($sasaran_nox[$sasaranx['id']])) {
                    // Simpan data baru jika belum ada di $sasaran_nox
                    $upSasaranx['rcsa_no'] = $rcsa_no;
                    $upSasaranx['create_user'] = $this->authentication->get_info_user('username');
                    $this->crud->crud_data(['table' => _TBL_RCSA_SASARAN, 'field' => $upSasaranx, 'type' => 'add']);
                    $sasaran_nox[$sasaranx['id']] = $this->db->insert_id();
                }
            }
        }
    }
