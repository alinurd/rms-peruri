			...
			<table class="display table table-bordered table-hover" id="tbl_register" style="font-size:85%;">
				...
				<tbody>
					...
					foreach($field as $key=>$row){
						...
						?>
						<tr>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$no;?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['lokasi']. $row['id_rcsa_detail'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['aktifitas'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['bahaya'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['resiko'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['kode_kondisi'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['kode_tr'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['kode_sd'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['kode_bp'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['kode_lp'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['kode_cp'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['severity'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['kode_occorunce'];?></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['score_resiko'];?></td>
							<td><span style="background-color:<?=$row['warna'];?>;color:<?=$row['warna_text'];?>;padding:8px;"><?=$row['tingkat'];?></span></td>
							<td rowspan="<?=count($row['mitigasi']);?>"><?=$row['isiRegulasi'];?></td>
							<td class="text-center" rowspan="<?=count($row['mitigasi']);?>"><?=$row['kode_penting'];?></td>
							<?php
