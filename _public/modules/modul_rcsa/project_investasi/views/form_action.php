...
echo form_open_multipart(base_url('project/save_action'),array('name'=>'form_action','id'=>'form_input_action','class'=>'form-inline'));?>
...
<div id="content_detail">
	<table class="table table-condensed no-margin-bottom" width="100%" cellspacing="0" cellpadding="0" border="0" id="tbl_action">
		<tbody>
			...
			<tr> <!-- proaktif -->
				<td width="25%" class="row-title  no-border vcenter text-left">
					<strong><?php echo lang('msg_field_proaktif');?></strong>
				</td>
				<td class="no-border vcenter">
					<input type="text" class="form-control p100" name="proaktif" id="proaktif" value="<?php echo $data['proaktif'];?>" style="width: 100%"/>
				</td>
			</tr>

			<tr> <!-- reaktif -->
				<td width="25%" class="row-title  no-border vcenter text-left">
					<strong><?php echo lang('msg_field_reaktif');?></strong>
				</td>
				<td class="no-border vcenter">
					<input type="text" class="form-control p100" name="reaktif" id="reaktif" value="<?php echo $data['reaktif'];?>"	style="width: 100%"/>
				</td>
			</tr>
			...
			<tr> <!-- pic -->
				...
				<td class="no-border vcenter">
					<?php 
						...
						// echo $isi_owner;
						echo form_dropdown('owner_no_action[]',$cbo_owner, $isi_owner,'multiple="multiple" class="select3 form-control" style="width:100%;"');?>
				</td>
			</tr>

			<tr> <!-- accountable_unit -->
				<td class="row-title  no-border vcenter text-left">
					<strong><?php echo lang('msg_field_responsible_unit');?></strong>
				</td>
				<td class="no-border vcenter">
					<?php 
					$isi_owners="";

					// if ($data['accountable_unit'] != "") {
					// 	$isi_owners = json_decode($data['accountable_unit']);
					// }
						//$isi_owners = json_decode($data['accountable_unit']);
						if( count($data['accountable_unit']) != 0 ) {
							$isi_owners = $data['accountable_unit'];
						}
						echo form_dropdown('owner_no_action_accountable[]',$cbo_owner, json_decode($isi_owners),'multiple="multiple" class="select3 form-control" style="width:100%;"');?>
				</td>
			</tr>
				...
				<td class="no-border vcenter">
					<?php if (!empty($data['attc'])){
						...
						echo ' <div class="alert alert-info fade in" id="attachment1">
                                  ...
                                  <strong><a target="_blank" href="'.base_url('project/get-download-file/'.$data['attc']).'">'.$data['attc'].'</a></stong>
							   </div>';
	...
	<div class="box-body">
		...
		?>	
		<span class="delete btn btn-warning btn-flat" data-content="Remove Data Event Detail" data-toggle="popover" url="<?php echo base_url('project/delete-action/'.$id_rcsa.'/'.$id_event_detail.'/'.$id_event_action);?>">
		...
		<a class="add btn btn-success btn-flat" data-content="Add New Data" data-toggle="popover" href="<?php echo base_url('project/risk-event/'.$id_rcsa.'/'.$id_event_detail);?>" >
...
<?php echo form_close();?>
<?php
$owner_name = form_dropdown('owner_no_action[]',$cbo_owner, '','class="select3 select4 form-control p100"');
$accountable_units = form_dropdown('owner_no_action_accountable[]',$cbo_owner, '','class="select3 select4 form-control p100"');
?>
