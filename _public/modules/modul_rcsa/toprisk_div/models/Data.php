...
class Data extends MX_Model {
	...
	function get_data_risk_register($data=0){
		$rows = $this->db->where_in('owner_no', $data)->where('period_no', _TAHUN_NO_)->where('sts_propose > 3')->get(_TBL_RCSA)->result_array();
		$sts=false;
		$sts_propose=1;
		...
		if (!$sts){
			...
			$rows = $this->db->where_in('owner_no', $data)->where('urgensi_no_kadiv',0)->where('period_no', _TAHUN_NO_)->where('sts_propose',3)->get(_TBL_VIEW_REGISTER)->result_array();
