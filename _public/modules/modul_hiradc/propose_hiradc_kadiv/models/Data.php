...
class Data extends MX_Model {
	...
	function get_data_risk_register($id){
		$rows = $this->db->where('rcsa_no', $id)->where('urgensi_no <> 0 and sts_propose=1')->get(_TBL_VIEW_REGISTER)->result_array();
		foreach($rows as &$row){
	...
	public function prop($rcsa_action_no, $idx)
	{
		...
		$this->db->set('urgensi_no', $idx)->where('id', $rcsa_action_no)->update(_TBL_RCSA_ACTION);
		return TRUE;
	}
}
