...
class Propose_Hiradc extends BackendController {
	var $tmp_data=array();
	...
	function send_propose(){
		$id = $this->input->post('edit_no');
		$this->crud->crud_data(array('table'=>_TBL_HIRADC, 'field'=>array('sts_propose'=>1),'where'=>array('id'=>$id),'type'=>'update'));
		$hasil['combo']=array();
	...
	function get_register(){
		...
		$data['id']=$id;
		$no=array(''=>' - ');
		for($x=1;$x<=count($data['field']);++$x){
			$no[]=$x;
		}
		$data['nourut']=$no;
		$hasil['combo']=$this->load->view('risk-register', $data, true);
		echo json_encode($hasil);
	}
