...
class Tingkat_Resiko extends BackendController {
	var $jml_pemakai=array();
	public function __construct()
	{
        parent::__construct();
		$this->kelCombo="tingkat-resiko";
		$this->set_Tbl_Master(_TBL_DATA_COMBO);
		
		$this->set_Open_Tab('Tingkat Resiko');
			...
			$this->addField(array('field'=>'param1', 'required'=>true, 'input'=>'color', 'size'=>10));
			$this->addField(array('field'=>'param2', 'required'=>true, 'input'=>'color', 'size'=>10));
		...
		$this->set_Table_List($this->tbl_master,'item_use','',10, 'center');
	...
	function listBox_ITEM_USE($row, $value){
		$result='';
		$value=$this->db->where('tingkat_resiko_no', $row['l_id'])->get(_TBL_LEVEL_COLOR_K3)->num_rows();
		if ($value>0)
