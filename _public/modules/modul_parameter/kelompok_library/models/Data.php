...
class Data extends MX_Model {
	...
	function get_library($id = 0, $key = 1)
	{
		$this->db->select(_TBL_LIBRARY . '.*, ' . 'bangga_map_kelevent' . '.event_no, ' . 'bangga_map_kelevent' . '.id as edit_no');
		$this->db->from('bangga_map_kelevent');
		$this->db->join(_TBL_LIBRARY, 'bangga_map_kelevent' . '.event_no=' . _TBL_LIBRARY . '.id');
		$this->db->where('bangga_map_kelevent' . '.library_no', $id);
