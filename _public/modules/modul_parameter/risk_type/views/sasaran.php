	...
	<tbody>
		...
		foreach ($field as $key => $row) {
			if ($this->uri->segment(2) == "view") {
				...
				$sasaran = form_textarea('data[]', $row['data'], "  maxlength='10000' class='form-control' disabled='disabled' rows='2' cols='5' style='overflow: hidden; height: 60px;'");
				...
				$edit = form_hidden('id_edit[]', $row['id']);
				$sasaran = form_textarea('data[]', $row['data'], "  maxlength='10000' class='form-control' rows='2' cols='5' style='overflow: hidden; height: 60px;'");
				// $strategi = form_textarea('strategi[]', $row['strategi'],"  maxlength='10000' class='form-control' rows='2' cols='5' style='overflow: hidden; height: 60px;'");
				// $kebijakan = form_textarea('kebijakan[]', $row['kebijakan'],"  maxlength='10000' class='form-control' rows='2' cols='5' style='overflow: hidden; height: 60px;'");
			}
			++$i;
			$jml = 0;
		...
		?>
			<tr>
				...
				<?php if ($this->uri->segment(2) == "view") : ?>
					<td></td>
				<?php else : ?>
					<td style="text-align:center;width:10%;"><a href=" <?= base_url() ?>risk-type/delete_subrisiko/<?php echo $row['id']; ?>/<?php echo $this->uri->segment(3); ?>"><i class="fa fa-cut" title="menghapus data"></i></a></td>
				<?php endif ?>
			</tr>
		<?php
		}
		$sasaran = form_textarea('data[]', '', "  maxlength='10000' class='form-control' rows='2' cols='5' style='overflow: hidden; height: 60px;'");
		$strategi = form_textarea('strategi[]', '', "  maxlength='10000' class='form-control' rows='2' cols='5' style='overflow: hidden; height: 60px;'");
		$kebijakan = form_textarea('kebijakan[]', '', "  maxlength='10000' class='form-control' rows='2' cols='5' style='overflow: hidden; height: 60px;'");
