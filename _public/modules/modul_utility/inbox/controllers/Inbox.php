...
class Inbox extends BackendController {
	public function __construct()
	{
		...
		$this->set_Table_List($this->tbl_master,'from', '', 20);
	...
	function listBox_FROM($rows, $value){
		if(array_key_exists($value, $this->list_from)){
			$value=$this->list_from[$value];
		}
		return $value;
	}
	...
	function POST_INSERT_PROCESSOR($id , $new_data){
		...
		if (is_array($new_data['l_to'])){
			if (count($new_data['l_to'])>1){
				foreach ($new_data['l_to'] as $key=>$row){
					if ($key>0){
						$upt=array();
						$upt['parent_no']=$id;
			...
			}elseif (count($new_data['l_to'])==1){
				$upt['from']=$this->authentication->get_Info_User('identifier');
				$upt['kelompok']=$kelompok;
				$upt['real_to']=$new_data['l_to'][0];
				$result = $this->crud->crud_data(array('table'=>$this->tbl_master, 'field'=>$upt, 'where'=>array('id'=>$id),'type'=>'update'));
			}
		}
		return true;
