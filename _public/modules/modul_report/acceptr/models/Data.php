...
class Data extends MX_Model {
    ...
    function grafik(){
		...
		foreach($rows as $key=>$row){
			...
			$dampak = '<b>Dampak</b></br>';
			$no = 1;
			foreach ($couse as $q => $v) {
				$dampak .= '<b>'.$no++.'. '.$v['description'].'<b><br>';
			}

			$penyebab = '<b>Penyebab</b></br>';
			$no = 1;
			foreach ($impact as $k => $w) {
				$penyebab .= '<b>'.$no++.'. '.$w['description'].'<b><br>';
			}

			$peristiwa[$row['sasaran_no']][$key]['id']=$row['id'];
			$peristiwa[$row['sasaran_no']][$key]['name']=$row['event_name'];
            $peristiwa[$row['sasaran_no']][$key]['children'] = $dampak.$penyebab ;
			// $jmlMitigasi = 0;
			// $jmlRealisasi = 0;
