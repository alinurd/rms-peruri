...
class Report_Rbb extends BackendController {
	var $table = "";
	var $post = array();
	var $sts_cetak = false;
	...
	public function __construct()
	{
		...
		$this->set_Open_Tab('Report Risk Register');
			...
			// $this->addField(array('field' => 'judul_assesment', 'input' => 'free', 'type' => 'free', 'show' => false, 'size' => 15));
		...
		$this->set_Table_List($this->tbl_master, 'nm_file', 'View Dokumen', 8, 'center',true,false);
	...
	{
		// doi::dump($row['l_judul_assesment']);
		$data = $row['l_nm_file']; 
 		if($data){
	...
	{		$data['file'] = $this->input->post('file');
		...
		// $data['file']="file";
		$result['register'] = $this->load->view('view_doc', $data, true);
		...
		function listBox_REGISTER($row, $value)
	{
		...
		$result = '<i class="fa fa-search showRegisterrbb pointer" data-id="' . $id . '" data-owner="' . $owner . '"></i>';
		return $result;
	}
