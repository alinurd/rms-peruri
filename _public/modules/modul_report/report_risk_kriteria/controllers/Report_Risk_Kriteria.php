...
class Report_Risk_Kriteria extends BackendController {
	var $table = "";
	var $post = array();
	var $sts_cetak = false;
	...
	public function __construct()
	{
		...
		$this->set_Open_Tab('RCSA');
			...
			$this->addField(array('field' => 'aksi', 'input' => 'free', 'type' => 'free', 'show' => false, 'size' => 15));
		...
		$this->set_Table_List($this->tbl_master, 'judul_assesment','Judul Assesment',20);
		$this->set_Table_List($this->tbl_master, 'name','Risk Owner');
		$this->set_Table_List($this->tbl_master, 'kriteria_probabilitas', 'Jml Probabilitas', 10, 'center');
		$this->set_Table_List($this->tbl_master, 'kriteria_dampak', 'Jml Dampak', 10, 'center');
		$this->set_Table_List($this->tbl_master, 'periode_name','Periode',10,'center');
		$this->set_Table_List($this->tbl_master, 'aksi', 'View Risk Kriteria', 10, 'center','false',false);
...
function cetak_kriteria()
	{
		...
		$nama = $nama = 'Risk-Kriteria-' . url_title($rows['name']);
		...
		$hasil = $this->load->view('list_risk_kriteria', $data, true);
		$cetak = 'register_' . $tipe;
		$this->$cetak($hasil, $nama);
	}
	function register_pdf($data, $nama = "Risk-Kriteria")
	{
