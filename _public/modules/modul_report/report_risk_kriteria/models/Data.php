...
class Data extends MX_Model {
	...
	function get_data_risk_criteria($id){
		$rows = $this->db->where('id', $id)->get(_TBL_RCSA_KRITERIA)->result_array();
		return $rows;
	}
		function get_data_risk_context($id){
		$rows = $this->db->where('id', $id)->get(_TBL_VIEW_RCSA)->result_array();
		return $rows;
	}
	...
	function cari_total_dipakai($id){
		...
		$rows = $this->db->select('rcsa_no, count(rcsa_no) as jml')->where_in('kriteria_type', 1)->group_by('rcsa_no')->get(_TBL_RCSA_KRITERIA)->result_array();
		$result['probabilitas']=[];
		$result['dampak']=[];
		foreach($rows as $row){
			$result['probabilitas'][$row['rcsa_no']]=$row['jml'];
		}
		$rows = $this->db->select('rcsa_no, count(rcsa_no) as jml')->where_in('kriteria_type', 2)->group_by('rcsa_no')->get(_TBL_RCSA_KRITERIA)->result_array();
		$result['dampak']=[];
		foreach($rows as $row){
			$result['dampak'][$row['rcsa_no']]=$row['jml'];
		}	
